...
test('provides desktop dialog and mobile drawer entry surfaces', () => {
  ...
  assert.match(
    formSource,
    /fund-code-badge[\s\S]*只记录本地买入事实，不会提交真实交易。[\s\S]*<section/,
  )
  assert.match(
    sellFormSource,
    /fund-code-badge[\s\S]*只记录本地卖出事实，不会提交真实交易。[\s\S]*<section/,
  )
})
...
test('keeps one shared confirmation footer for each transaction surface', () => {
  ...
  assert.match(formSource, /@submit\.prevent="emit\('save'\)"/)
  assert.match(sellFormSource, /@submit\.prevent="emit\('save'\)"/)
...
test('uses TDesign controls while preserving the transaction value contract', () => {
  for (const source of [formSource, sellFormSource]) {
    ...
    assert.match(source, /String\(\$event\)/)
  }
})
