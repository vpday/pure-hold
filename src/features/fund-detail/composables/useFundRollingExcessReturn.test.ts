...
import { useFundRollingExcessReturn } from './useFundRollingExcessReturn.ts'

test('loads lazily, defaults to one year and changes all ranges without requesting', async () => {
  const calls: string[] = []
  const session = useFundRollingExcessReturn(historySource(calls), benchmarkSource(calls))

  session.initialize('161725')
  assert.equal(session.selectedRange.value, 'n')
  assert.deepEqual(calls, [])
  await session.activate()
  assert.deepEqual(calls.sort(), ['benchmark', 'distribution', 'net-values'])
  assert.equal(session.data.value?.status, 'ready')

  for (const range of ['3n', '5n', 'ln', 'n'] as const) session.selectRange(range)
  assert.deepEqual(calls.sort(), ['benchmark', 'distribution', 'net-values'])
...
test('retries an initial failure and preserves successful data after a failed refresh', async () => {
  ...
  const session = useFundRollingExcessReturn(dataSource, benchmarkSource([]))
  session.initialize('161725')

  await session.activate()
  assert.equal(session.error.value, '滚动超额加载失败，请稍后重试')
...
test('surfaces source quality warnings without blocking a usable result', async () => {
  ...
  const session = useFundRollingExcessReturn(history, benchmark)
  session.initialize('161725')
...
test('shares in-flight requests and successful caches with another rolling session', async () => {
  const calls: string[] = []
  const history = historySource(calls)
  const benchmark = benchmarkSource(calls)
  const first = useFundRollingExcessReturn(history, benchmark)
  const second = useFundRollingExcessReturn(history, benchmark)
