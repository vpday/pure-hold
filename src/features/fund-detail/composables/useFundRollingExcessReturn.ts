...
export function useFundRollingExcessReturn(
  historyDataSource: FundHistoryDataSource,
  benchmarkDataSource: FundBenchmarkDataSource,
) {
  const currentFundCode = ref<string>()
  const data = shallowRef<FundRollingExcessReturnResult>()
  const error = ref('')
  const isLoading = ref(false)
  const selectedRange = ref<FundRollingExcessRange>(defaultFundRollingExcessRange)
  const warning = ref('')
  let active = false
  ...
  function selectRange(range: FundRollingExcessRange): void {
  ...
  function resetState(): void {
    ...
    selectedRange.value = defaultFundRollingExcessRange
    warning.value = ''
  }
