...
import { useFundDrawdownComparison } from './useFundDrawdownComparison.ts'
...
test('loads lazily, defaults to one year and changes all ranges without requesting', async () => {
  const calls: string[] = []
  const session = useFundDrawdownComparison(historySource(calls), benchmarkSource(calls))
  ...
  for (const range of ['n', '3n', '5n', 'ln'] as const) session.selectRange(range)
  assert.deepEqual(calls.sort(), ['benchmark', 'distribution', 'net-values'])
...
test('retries an initial failure and preserves successful data after a failed refresh', async () => {
  ...
  const session = useFundDrawdownComparison(history, benchmarkSource([]))
  session.initialize('161725')

  await session.activate()
  assert.equal(session.error.value, '回撤对比加载失败，请稍后重试')
...
test('surfaces source quality warnings without blocking usable data', async () => {
  const history = useFundHistoryDataSource({
  ...
  const session = useFundDrawdownComparison(history, benchmark)
  session.initialize('161725')
...
test('closes without aborting other subscribers to the shared sources', async () => {
...
function benchmarkHistory(endDate: string, invalid = false): IndexPerformanceHistory {
  return {
    ...
    points: [
      ...
      { date: '2024-01-31', value: 1800 },
      { date: '2026-01-31', value: 1700 },
