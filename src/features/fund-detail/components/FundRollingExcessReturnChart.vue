...
function summaryColor(
  color: FundRollingExcessReturnChartModel['summary'][number]['color'],
): string {
...
function summaryValueColor(
  trend: FundRollingExcessReturnChartModel['summary'][number]['trend'],
): string | undefined {
  ...
  <div class="relative min-h-80 w-full">
    ...
    >
      <div v-for="item in model.summary" :key="item.label" class="summary-item">
        ...
        <span class="shrink-0" :style="{ color: summaryValueColor(item.trend) }">
          {{ item.valueText }}
        </span>
      </div>
    </div>
