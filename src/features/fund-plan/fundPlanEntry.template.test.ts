...
test('exposes plan editing and non-payment execution wording', () => {
  ...
  assert.match(formSource, /暂停计划/)
  assert.match(formSource, /删除计划/)
...
test('uses one plan title and one final save action across both surfaces', () => {
  ...
  assert.match(entrySource, /function submitPlan\(\): void/)
  assert.match(entrySource, /@click="submitPlan"/)
  assert.match(formSource, /function submit\(\): void/)
  assert.match(formSource, /@submit\.prevent="submit"/)
  assert.match(formSource, /defineExpose\(\{ submit \}\)/)
